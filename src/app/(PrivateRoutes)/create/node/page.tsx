...
export default function NodeCreationForm({ initialData, isEditMode = false }: NodeCreationFormProps) {
  ...
  const toggleVisibility = (index: number) => {
    setFormState((prev) => ({
      ...prev,
      sshKeys: prev.sshKeys.map((key, i) => (i === index ? { ...key, isVisible: !key.isVisible } : key)),
    }))
  }
        ...
        <Card className="bg-neutral-100 shadow-none border-none">
          <CardHeader className="text-center">
            <CardTitle className="text-lg lg:text-2xl font-semibold">
              {isEditMode ? "Edit Node" : "Create a New Node"}
            </CardTitle>
          </CardHeader>
                    ...
                    {formState.securityRules.map((rule, index) => (
                      <div key={index} className="p-4 border rounded-md space-y-2">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <Button type="button" variant="ghost" size="icon" onClick={() => removeSecurityRule(index)}>
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        </div>
